...
const config: AddOnReleaserConfiguration = CommentJSON.parse(readFileSync(configPath).toString()) as any;
            ...
            for (const item of folderContents) {
                if (item.isFile() && item.name !== "add-on-releaser-config.json") {
                    ...
                    if (item.name === "manifest.json" && currentPath === "" && folder === "") {
                        const newManifest: ManifestSchema = CommentJSON.parse(
                        ...
                        if (config.release_version_format !== "current") {
                            if (typeof newManifest.header.version === "string") {
                                if (!config.release_version_format || config.release_version_format === "tuple") {
                                    ...
                                    newManifest.header.name = newManifest.header.name.replace(
                        ...
                        let text: string = CommentJSON.stringify(newManifest, null, 4);
                        text =
                        ...
                        try {
                            manifestData = CommentJSON.parse(text) as any;
                        } catch (e) {
                            throw new SyntaxError(
                                `Failed to parse generated manifest "${name}" in pack "${pack.path}": ${e}${
